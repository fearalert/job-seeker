           ...
           { user?.role === ROLES.JOB_SEEKER &&
            ...
            <div className="sm:col-span-2 bg-gray-50 p-3 rounded-lg">
              ...
              <Link
                ...
                download={user?.resume && `${user?.name}_resume.pdf`}
                target={user?.resume && "_blank"}
                rel={user?.resume && "noopener noreferrer"}
                className= {`${user?.resume ? "text-primary hover:underline" : "text-destructive"}`}
              >
              {user?.resume ? "Download Resume" : "No Resume Found"}
              </Link>
            </div>
            }
        </div>
